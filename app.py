...
st.set_page_config(page_title="MAYA AI - Smart Sniper Engine", layout="wide")

st.title("MAYA AI 🦅: Smart Sequential Sniper ⚡")
st.markdown("Aapke naye Master Logic par aadharit: **Step-by-Step Filtering!** Pehle sirf un Timeframes ko uthao jo kal paas aur parso fail hue. Phir sirf unhi bache hue timeframes par pichli history aur Max-Fail check karo. No Hang, No Delay!")
...
if uploaded_file is not None:
    try:
        ...
        # ==========================================
        # AAPKA NAYA: SMART SEQUENTIAL FILTER (Fastest)
        # ==========================================
        @st.cache_data
        def get_sniper_timeframe_smart(history_tuple, dates_tuple, max_lookback=45):
            ...
            # 🚀 STEP 1: FAST ELIMINATION (Kal Pass & Parso Fail hona zaroori)
            # Jo yahan fail hoga uski lambi history check karke time waste nahi karenge
            for tf in range(1, min(max_lookback, len(history_list)-10)):
                if not check_hit_for_day(len(history_list)-1, tf): continue # Kal Pass nahi hua? Reject!
                if check_hit_for_day(len(history_list)-2, tf): continue     # Parso bhi Pass tha? Reject! (Humein broken streak chahiye)
                
                surviving_tfs.append(tf)

            valid_tfs = []
            
            # 🚀 STEP 2: HEAVY CHECKS ON SURVIVORS ONLY (Yeh sirf 3 se 8 timeframes honge)
            for tf in surviving_tfs:
                # Streak fail count before Kal
                fail_count_before_pass = 1 # Hum jaante hain Parso (Day -2) fail tha
                ...
                # Poori history ka array sirf isi TF ka banega
                hit_history = [check_hit_for_day(i, tf) for i in range(15, len(history_list))]
                
                # Rule 4 & 5: Jan-Apr Score aur Max Historical Fails
                jan_apr_score = 0
                max_hist_fails = 0
                current_fails = 0
            ...
            # SORTING: Jiska Max Hist Fail sabse kam ho, wo winner!
        ...
        for shift in shift_order:
            ...
            # YAHAN SE MAIN DATA NAHI KAAT RAHA. FULL HISTORY USE HOGI!
            history_today = shift_data[shift].astype(int).tolist()
            dates_today = shift_data['DATE'].tolist()
            
            if len(history_today) >= 60:
                st.markdown("---")
                st.subheader(f"🧩 Shift: {shift} (Smart Sequential Mode ⚡)")

                with st.spinner(f"Fatak se load ho raha hai... Step-by-Step elimination shuru!"):
                ...
                if fails_before > 0:
                    ...
                    date_text = f"⚠️ (Sniper condition match nahi hui)."
                ...
                if current_fail_streak == 0:
                    ...
                    st.markdown(f"<div style='background:#FF4B4B; padding:10px; border-radius:8px; border: 2px solid #c82333; text-align:center; color:white; margin-bottom:10px; box-shadow: 0 0 10px #FF4B4B;'>"
                                f"🔥 <b>GEAR SHIFTED ({current_fail_streak} Din Se Fail)</b><br>Smart Sequential Filter lagaya gaya hai!</div>", unsafe_allow_html=True)
                ...
                with c_stat:
                    st.markdown(f"<div style='border:2px solid #00FF7F; padding:10px; border-radius:8px; background:#00FF7F15; font-size:14px;'>"
                                f"<b style='color:#00FF7F; font-size:16px;'>🦅 SMART SNIPER INTELLIGENCE</b><br>"
