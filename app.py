...
st.set_page_config(page_title="MAYA AI - 2-Fail Shifter", layout="wide")

st.title("MAYA AI 🦅: Fast Cache & '2-Fail' Gear Shifter Engine")
st.markdown("Aapke naye niyam par aadharit: **0 Fail aur 1 Fail par SAME Timeframe rahega!** Timeframe (Gear) sirf tab badlega jab lagatar **2 baar fail** ho chuka ho. Lookback wapas 45 din kar diya gaya hai aur code ko super-fast banaya gaya hai!")
...
if uploaded_file is not None:
    try:
        ...
        for shift in shift_order:
            ...
            if len(history_today) < 60: continue
            
            st.markdown("---")
            st.subheader(f"🧩 Shift: {shift}")

            with st.spinner(f"Super-Fast Memory Engine chal raha hai..."):
                
                today_tiers = get_all_tiers(history_today)
                player_trap_nums = detect_player_load_trap(tuple(history_today))
                
                # Pata lagao aaj ki FAIL STREAK kitni hai (Main Logic ke hisaab se)
